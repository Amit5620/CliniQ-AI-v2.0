...
Handles Pneumonia image processing and prediction functionality
"""
...
def handle_pneumonia_prediction(user):
    """Handle pneumonia prediction with comprehensive processing"""
    last_conv_layer_name = "Top_Conv_Layer"
    ...
    try:
        ...
        pneumonia_form_data = session.get('pneumonia_form_data', {})
        file = pneumonia_form_data.get('file')
        if not file:
            file = request.files.get('file')
        
        if not file:
            flash('Please upload an Pneumonia image', 'error')
            return redirect(request.referrer or '/pneumonia_predict')
        
        filename = secure_filename(file.filename)
        filename = f"pneumonia_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{filename}"
        ...
        patient_name = pneumonia_form_data.get('patient_name') or request.form.get('patient_name')
        patient_age = int(pneumonia_form_data.get('patient_age') or request.form.get('patient_age'))
        patient_gender = pneumonia_form_data.get('patient_gender') or request.form.get('patient_gender')
        patient_id = f"PNE-{user}-{datetime.now().strftime('%Y%m%d%H%M%S')}"
        doctor_name = pneumonia_form_data.get('doctor_name') or request.form.get('doctor_name')
        doctor_type = pneumonia_form_data.get('doctor_type') or request.form.get('doctor_type')
        symptoms = pneumonia_form_data.get('patient_symptoms') or request.form.get('patient_symptoms')
        disease_type = pneumonia_form_data.get('disease_type') or 'Pneumonia'
        ...
        def process_result(result, suffix):
            ...
            # For  pneumonia, the predicted image is the gradcam
        ...
        db.session.rollback()
        flash(f'Error processing Pneumonia: {str(e)}', 'error')
        return None
